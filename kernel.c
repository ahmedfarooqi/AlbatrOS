...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        file_sys_start = (uint32_t)(mod->mod_start);
        file_sys_end = (uint32_t)(mod->mod_end);
    ...
    sti();
    rtc_initialize();
    file_system_init(file_sys_start, file_sys_end);

    clear();
    // lock_main = 1;  
    // t2_launch = 1;
    // t3_launch = 1;//move up before shell
    active_terminal = 0;
    b8_map(0);
    clear();
    // visible_terminal = 0;
